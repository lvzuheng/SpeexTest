...
public abstract class IdleEvent {
    ...
    private long deviation = 10;//代码运行误差
...
//                        Log.e("idle","write:"+System.currentTimeMillis()+","+writeIdleTime+","+WRITEIDLE+","+((System.currentTimeMillis() - writeIdleTime) % WRITEIDLE));
    ...
    protected void writeIdleRuning(long delay) {
        if (delay <= 0)
            return;

//        Log.e("WRITEIDLE","write111:"+delay);
...
//                        Log.e("WRITEIDLE", "idle:" + current +","+aLong);
                        if (current <= deviation) {
//                            Log.d("WRITEIDLE", "测试类型:" + Event.WRITE_IDLE);
